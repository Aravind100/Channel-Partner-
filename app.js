...
function getSiteConfig(){
  // config.js declares SITE_CONFIG as a top-level const, which is not guaranteed
  // to be exposed as window.SITE_CONFIG in every browser.
  return (typeof SITE_CONFIG !== "undefined" ? SITE_CONFIG : (window.SITE_CONFIG || {}));
}

function wa(message){
  const c = getSiteConfig();
  const n = String(c.whatsappNumber || "").replace(/\D/g,"");
  if(!n || n.includes("XXXXXXXX") || n.length < 10) {
    alert("WhatsApp is not configured correctly. Please check whatsappNumber in config.js.");
    return;
  }

  const url = `https://wa.me/${n}?text=${encodeURIComponent(String(message || "Hello, I would like to know more about your financial services."))}`;

  // Navigate directly instead of window.open(). This avoids popup blockers,
  // especially on mobile browsers and GitHub Pages.
...
document.addEventListener("click",e=>{
  ...
  if(el){
    e.preventDefault();
    e.stopPropagation();
    wa(el.getAttribute("data-wa-message"));
    return;
  }
